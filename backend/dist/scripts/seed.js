...
async function seed() {
    ...
    console.log(`[Seed] Connected to database: "${mongoose_1.default.connection.name}"`);
    // ---- 1. Roles -----------------------------------------------------------
    ...
    await Role_js_1.Role.updateOne({ key: 'super_admin' }, {
    ...
    await Role_js_1.Role.updateOne({ key: 'editor' }, {
        $setOnInsert: {
    ...
    const superAdminRole = await Role_js_1.Role.findOne({ key: 'super_admin' });
    ...
    if (!existingAdmin) {
        const passwordHash = await bcryptjs_1.default.hash(adminPassword, await bcryptjs_1.default.genSalt(10));
    ...
    for (const cat of CATEGORIES) {
        await Category_js_1.Category.updateOne({ key: cat.key }, { $set: { name: cat.name, displayOrder: cat.displayOrder, isActive: cat.isActive } }, { upsert: true });
    }
    console.log(`[Seed] ${CATEGORIES.length} Categories seeded`);
    // ---- 4. Brands ---------------------------------------------------------
    for (const br of BRANDS) {
        await Brand_js_1.Brand.updateOne({ slug: br.slug }, { $set: { name: br.name, displayOrder: br.displayOrder, isActive: br.isActive } }, { upsert: true });
    }
    ...
    await Product_js_1.Product.deleteMany({ categoryKey: { $nin: ['ethical', 'surgical', 'otc', 'critical'] } });
    let srNo = 1;
    for (const prod of PRODUCTS) {
        await Product_js_1.Product.updateOne({ slug: prod.slug }, {
            $set: {
                srNo: srNo++,
                name: prod.name,
    ...
    const totalProds = await Product_js_1.Product.countDocuments();
    console.log(`[Seed] ${totalProds} Pharmaceutical Products active in database`);
    // ---- 6. FAQs -----------------------------------------------------------
    await Entities_js_1.Faq.deleteMany({});
    for (const f of FAQS) {
        await Entities_js_1.Faq.create(f);
    }
    console.log(`[Seed] ${FAQS.length} FAQs seeded`);
    // ---- 7. Testimonials ---------------------------------------------------
    await Entities_js_1.Testimonial.deleteMany({});
    for (const t of TESTIMONIALS) {
        await Entities_js_1.Testimonial.create(t);
    }
    console.log(`[Seed] ${TESTIMONIALS.length} Testimonials seeded`);
    // ---- 8. Blogs ----------------------------------------------------------
    await Entities_js_1.Blog.deleteMany({});
    for (const b of BLOGS) {
        await Entities_js_1.Blog.create(b);
    }
    console.log(`[Seed] ${BLOGS.length} Blogs seeded`);
    // ---- 9. Website CMS Pages & Sections -----------------------------------
    for (const p of PAGES) {
        await WebsiteEditor_js_1.Page.updateOne({ key: p.key }, { $set: p }, { upsert: true });
    }
    console.log(`[Seed] ${PAGES.length} CMS pages ready`);
    for (const s of SECTIONS) {
        await WebsiteEditor_js_1.Section.updateOne({ pageKey: s.pageKey, key: s.key }, { $set: { name: s.name, description: s.description, order: s.order, fields: s.fields } }, { upsert: true });
        const section = await WebsiteEditor_js_1.Section.findOne({ pageKey: s.pageKey, key: s.key });
        const content = await WebsiteEditor_js_1.SectionContent.findOne({ pageKey: s.pageKey, sectionKey: s.key });
        if (!content) {
