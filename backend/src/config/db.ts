...
export async function connectDB() {
  ...
  if (!dbName) {
    throw new Error(
      `[Database] MONGODB_URI has no database name. It must end in "/${EXPECTED_DB_NAME}".`
    );
  }

  if (dbName.toLowerCase() !== EXPECTED_DB_NAME.toLowerCase()) {
    throw new Error(
      `[Database] Refusing to connect: MONGODB_URI points at database "${dbName}", ` +
  ...
  console.log(`[Database] MongoDB connected — database "${mongoose.connection.name}"`);
}
